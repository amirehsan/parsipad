...
- **The card shows which direction it translated**, resolved from the result rather than from the guess made before the request, so a Persian word typed in Latin letters reads FA to EN rather than the other way round. A swap control next to it re-translates with the opposite source, and the correction is remembered for the rest of the session so a repeated mis-detection is corrected once rather than every time.
