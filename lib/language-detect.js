...
/**
 * ParsiPad supports Persian and English only. This helper guards the
 * translate/polish/dictionary entry points so input in Russian, Chinese,
 * Japanese, etc. surfaces a clear "unsupported" message instead of being
 * silently mis-translated or hallucinated against by the LLM.
 *
 * Detection is conservative:
 *  - Pure-numbers / pure-punctuation strings are treated as "no language" and
 *    accepted (they're round-tripped verbatim).
 *  - Mixed scripts (e.g. an English sentence with a Persian word in it) are
 *    accepted; the dominant script wins.
 *  - Only strong signals for non-fa/non-en scripts (CJK, Cyrillic, Hebrew,
 *    Greek, Thai, Devanagari) cause a rejection.
 *
 * @param {string} text
