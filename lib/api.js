...
export async function translate(text, sourceLang = 'auto', withGrammar = false) {
  ...
  // Detect if this is a numbered batch (for page translation)
  const isNumberedBatch = /^\[1\]\s/.test(text);

  let userPrompt;
  if (isNumberedBatch) {
    userPrompt = `Translate each numbered item from ${fromName} to ${toName}. IMPORTANT: Keep the [1], [2], etc. markers at the start of each line. Output format must be exactly:
