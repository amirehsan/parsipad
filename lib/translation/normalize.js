...
/**
 * Batch mode normalization: page translation sends one item per line as
 * `[n] text` and depends on both the markers and the line structure
 * surviving the round trip, so this must not drop bracketed numbers or
 * join line breaks the way normalizeInput does for prose.
 * @param {unknown} text
 * @returns {string}
 */
function normalizeBatch(text) {
  if (!text) return '';
  let s = String(text).replace(/\r\n?/g, '\n');
  s = stripInvisibles(s);
  s = s.replace(/[ \t]{2,}/g, ' ');
...
/**
 * Mode-appropriate normalization. Batch keeps `[n]` markers and one line
 * per item (see normalizeBatch); every other mode behaves exactly like
 * normalizeInput.
 * @param {unknown} text
 * @param {string} mode
 * @returns {string}
 */
export function normalizeForMode(text, mode) {
  return mode === MODES.BATCH ? normalizeBatch(text) : normalizeInput(text);
}
