import { DOCUMENT_SYSTEM_PROMPT, ERROR_MESSAGES } from './constants.js';
...
/**
 * Translate a single chunk of text using the selected AI provider
 ...
 */
export async function translateChunk(chunk, sourceLang, _targetLang) {
  ...
  try {
    const result = await provider.complete({
      ...
      userPrompt: `${directionPrompt}\n\n${chunk}`,
      maxTokens: provider.config.maxTokens * 2, // Allow more tokens for document chunks
      apiKey
    });
