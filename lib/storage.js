 ...
 * @param {string} item.type - 'translation' or 'polish'
 * @param {string} item.original - Original text
 * @param {string} item.saved - Saved translation or polish variant
 * @param {string} [item.variant] - For polish: 'professional', 'conversational', or 'concise'
 * @param {string} [item.direction] - For translation: 'en-to-fa' or 'fa-to-en'
...
export async function addFavorite(item) {
  ...
  const exists = favorites.some(f =>
    f.original === item.original && f.saved === item.saved
  );
 ...
 */
export async function isFavorite(original, saved) {
  const favorites = await getFavorites();
  return favorites.find(f => f.original === original && f.saved === saved) || null;
}
