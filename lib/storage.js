...
export async function setProviderApiKey(provider, key) {
  ...
  if (key) {
    const prefixes = Array.isArray(config.keyPrefix) ? config.keyPrefix : [config.keyPrefix];
    if (!prefixes.some(prefix => key.startsWith(prefix))) {
      const prefixStr = prefixes.join('" or "');
      throw new Error(`Invalid ${config.name} API key format. Key should start with "${prefixStr}"`);
    }
  }
