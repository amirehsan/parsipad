...
(function () {
  try {
    // 'light' rather than 'system' on a fresh install: the extension should
    // open in its default presentation until the user picks a side, and a
    // dark OS should not decide that for them.
    const stored = localStorage.getItem('pp-theme') || 'light';
