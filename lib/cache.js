...
class TranslationCache {
  ...
  /**
   * Get cached translation. Returns the full rich-context payload when
   * present so cache hits render the "Did you mean" hint, alternatives,
   * examples, and nuance just like a fresh API response.
   *
   * @param {string} text - Source text
   * @param {string} provider - Provider id used for this request
   * @param {string} sourceLang - Source language hint
   * @returns {Promise<{translation: string, direction: string, corrections?: Array, alternatives?: string[], examples?: Array, nuance?: string}|null>}
   */
  ...
  /**
   * Store translation in cache. Optional rich-context fields (corrections,
   * alternatives, examples, nuance) are persisted so subsequent cache hits
   * surface the same "Did you mean" / Linguistic context UI the user saw
   * on the first hit.
   *
   * @param {string} text - Source text
   ...
   * @param {string} sourceLang - Source language hint
   * @param {object} [rich={}] - Optional rich-context fields
   * @param {Array}  [rich.corrections]
   * @param {string[]} [rich.alternatives]
   * @param {Array}  [rich.examples]
   * @param {string} [rich.nuance]
   */
  async set(text, translation, direction, provider, sourceLang = 'auto', rich = {}) {
    const hash = await this.hashKey(text, provider, sourceLang);
    const cache = await this.loadCache();
    ...
    // Only store the rich fields when they're actually populated, so cache
    // entries stay compact for long-form translations that don't carry them.
    if (Array.isArray(rich.corrections) && rich.corrections.length) entry.corrections = rich.corrections;
    if (Array.isArray(rich.alternatives) && rich.alternatives.length) entry.alternatives = rich.alternatives;
    if (Array.isArray(rich.examples) && rich.examples.length) entry.examples = rich.examples;
    if (typeof rich.nuance === 'string' && rich.nuance.trim()) entry.nuance = rich.nuance;
